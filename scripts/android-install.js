...
module.exports = function (context) {
    ...
    var targetDir  = path.join(projectRoot, "platforms", "android", "src", "cn", "zxj", "cordova", "UmengAnalyticsPlugin");
        targetFile = path.join(targetDir, "UmengAnalyticsPlugin.java");
    ...
    if (['after_plugin_add', 'after_plugin_install', 'after_platform_add'].indexOf(context.hook) === -1) {
        ...
        // sync the content
        fs.readFile(path.join(context.opts.plugin.dir, 'src', 'android', 'UmengAnalyticsPlugin.java'), {encoding: 'utf-8'}, function (err, data) {
            if (err) {
                throw err;
            }
